...
TEST_DIR=$SCRIPT_DIR/${TEST:-maven-100-artifacts}
...
function run_test() {
    run_dir=$1
    if [[ -f "$run_dir"/.mvn/extensions.xml ]]; then
        sed -i 's/<version>[^<]*<\/version>/<version>2\.2\.6-SNAPSHOT<\/version>/' "$run_dir"/.mvn/extensions.xml
    fi

    cmd="mvn $MVN_ACTION -B -Dmaven.test.skip=true -Dmaven.repo.local=/src/_repo $MVN_FLAGS"
    if [[ -n "MVN_IMAGE" ]]; then
